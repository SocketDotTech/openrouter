/**
 * Verifies deployed RFQVaultExecutor source code on block explorers.
 *
 * Usage:
 *   OWNER_ADDRESS=0x... SOLVER_SIGNER_ADDRESS=0x... npm run verify:rfq-vault-executor
 *   OWNER_ADDRESS=0x... SOLVER_SIGNER_ADDRESS=0x... npm run verify:rfq-vault-executor -- polygon base
 */
...
import { readRFQVaultExecutorAddress } from './rfqVaultExecutorAddresses';
...
interface RFQVaultExecutorDeploymentRecord {
  chainId: number;
  network: string;
  address: string;
}
...
const CONTRACT_ID = 'src/executors/RFQVaultExecutor.sol:RFQVaultExecutor';
...
const EXPLORERS_BY_CHAIN_ID = new Map<number, ExplorerConfig>([
  ...
  [9745, { verifier: 'custom', chainArg: '1', verifierUrl: 'https://api.routescan.io/v2/network/mainnet/evm/9745/etherscan/api' }],
...
function loadStandardJsonInput(): string {
  const buildInfoDir = resolve(process.cwd(), 'out', 'build-info');
  const buildInfoPath = join(buildInfoDir, '701184e0c43d8ca1.json');
...
async function readDeploymentRecords(): Promise<RFQVaultExecutorDeploymentRecord[]> {
  const records: RFQVaultExecutorDeploymentRecord[] = [];
  ...
  for (const network of RECEIVER_DEPLOY_NETWORKS) {
    ...
    const address = await readRFQVaultExecutorAddress(network.name);
    if (!address) {
      throw new Error(`Missing RFQVaultExecutor address for ${network.name}`);
    }
    if (!ADDR_HEX_RE.test(address)) {
      throw new Error(
        `Invalid RFQVaultExecutor address for ${network.name}: ${address}`,
      );
    }
...
async function main() {
  ...
  if (records.length === 0) {
    throw new Error('No RFQVaultExecutor deployment records selected');
  }
