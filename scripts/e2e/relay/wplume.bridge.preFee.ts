/**
 * Route:  Plume WPLUME → Base USDC via Relay.link (no on-router swap)
 * Function: bridge (simple bridge entrypoint)
 * Fee: preFee — FEE_BPS of inputAmount WPLUME deducted before bridge
 *
 * Matches Relay UI:
 *   https://relay.link/bridge/base?fromChainId=98866
 *   &fromCurrency=0xea237441c92cae6fc17caaf9a7acb3f953be4bd1
 *   &toCurrency=0x833589fcd6edb6e08f4c7c32d4f71b54bda02913
 *
 * Usage:
 *   PRIVATE_KEY=0x... ts-node scripts/e2e/relay/wplume.bridge.preFee.ts
 *
 * Optional: INPUT_AMOUNT=<wei> to bridge a fixed WPLUME amount instead of wallet balance minus dust.
 */
...
const ROUTER_PLUME = routerAddressForChain(CHAIN_IDS.PLUME);
const WPLUME_DECIMALS = 18;
/** Leave 0.01 WPLUME in wallet for dust / rounding. */
...
async function main(): Promise<void> {
  ...
  const provider = new ethers.JsonRpcProvider(RPC.PLUME);
  const signer = new ethers.Wallet(privateKey, provider);
  const signerAddress = await signer.getAddress();

  const inputToken = TOKENS.WPLUME_PLUME;
  const { balance: walletBalance } = await getWalletErc20Balance(inputToken, signerAddress, provider);
  if (walletBalance === 0n) {
    throw new Error(`Signer ${signerAddress} has zero WPLUME on Plume`);
  }
  ...
  console.log(`Router:          ${ROUTER_PLUME}`);
  console.log(`WPLUME balance:  ${ethers.formatUnits(walletBalance, WPLUME_DECIMALS)}`);
  console.log(`Input amount:    ${ethers.formatUnits(inputAmount, WPLUME_DECIMALS)} WPLUME`);
  console.log(`Pre-bridge fee:  ${ethers.formatUnits(feeAmount, WPLUME_DECIMALS)} WPLUME (${FEE_BPS} bps)`);
  console.log(`Net to bridge:   ${ethers.formatUnits(bridgeAmount, WPLUME_DECIMALS)} WPLUME → Base USDC`);

  console.log('Fetching Relay.link quote (Plume WPLUME → Base USDC)...');
  const quote = await fetchRelayQuoteV2({
    routerAddress: ROUTER_PLUME,
    recipient: signerAddress,
    originChainId: CHAIN_IDS.PLUME,
    destinationChainId: CHAIN_IDS.BASE,
    originCurrency: TOKENS.WPLUME_PLUME,
  ...
  await ensureRouterErc20Balance(signer, inputToken, ROUTER_PLUME);
  ...
  logTxnSummary('Plume WPLUME → Base USDC — Relay — bridge preFee', CHAIN_IDS.PLUME, receipt);
}
