 ...
 * (output lands in router for balanceOf delta measurement).
 *
 * The quote is for swapInput (inputAmount − preFeeAmount) so the 0x calldata matches the
 * router's approval amount at execution time.
 *
 * USDC lands in signer's wallet on Polygon.
 *
 * Usage:
 *   PRIVATE_KEY=0x... ZEROX_API_KEY=... ts-node scripts/e2e/swap/zerox.preFee.balanceOf.ts
 */
 ...
 * and receives the output for balanceOf delta measurement.
 *
...
async function main() {
  ...
  // quote for the reduced swap input so 0x calldata encodes the correct sell amount
  ...
  const { swapTarget, swapData, buyAmount, minBuyAmount, value } =
    await fetchZeroXQuote(swapInput, ROUTER_POLYGON, ROUTER_POLYGON, signerAddress);
  ...
  logTxnSummary(
    `Polygon AAVE → USDC — 0x preFee/balanceOf`,
    CHAIN_IDS.POLYGON,
    receipt,
  );
