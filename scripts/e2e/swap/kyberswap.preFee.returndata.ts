 ...
 *
 * Pre-fee + returndata: swap output must go to `receiver` (signer); the router decodes amount from
 * returndata. Quote uses swapInput = inputAmount − fee so calldata matches the post-fee swap size.
 *
 * Kyber build: sender = router (executor), recipient = user (net output destination).
 *
 * USDC lands in signer's wallet on Polygon.
 *
 * Usage:
 *   PRIVATE_KEY=0x... KYBERSWAP_API_KEY=... ts-node scripts/e2e/swap/kyberswap.preFee.returndata.ts
 */
 ...
 * Pre-fee returndata: router executes; tokens are sent to `outputRecipient` (user).
 */
...
async function main() {
  ...
  const { ksRouter, swapData, estimatedOut, minAmountOut, value } =
    await fetchKyberSwapQuote(swapInput, ROUTER_POLYGON, signerAddress);
  ...
  logTxnSummary(
    `Polygon AAVE → USDC — kyberswap preFee/returndata`,
    CHAIN_IDS.POLYGON,
    receipt,
  );
