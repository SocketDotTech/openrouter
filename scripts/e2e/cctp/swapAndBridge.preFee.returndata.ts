 ...
 *
 * Usage:
 *   PRIVATE_KEY=0x... ts-node scripts/e2e/cctp/swapAndBridge.preFee.returndata.ts
 */
...
// pre-fee (0x00) | bridge amount at byte offset 4 (depositForBurn amount param)
const FLAGS = bridgeAmountPositionFlag(4);
...
async function main() {
  ...
  console.log(
    `Flags:         0x${FLAGS.toString(
      16
    )} (pre-fee, returndata, bridge-amount-pos=4)`
  );
  ...
  logTxnSummary(
    `Polygon AAVE → Base USDC (CCTP) — swapAndBridge preFee/returndata`,
    CHAIN_IDS.POLYGON,
    receipt
  );
