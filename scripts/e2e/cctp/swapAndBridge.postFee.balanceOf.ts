 ...
 *
 * Usage:
 *   PRIVATE_KEY=0x... ts-node scripts/e2e/cctp/swapAndBridge.postFee.balanceOf.ts
 */
...
// post-fee (0x01) | balance-of (0x02) | bridge amount at byte offset 4 (depositForBurn amount param)
const FLAGS = 0x03n | bridgeAmountPositionFlag(4);
...
async function main() {
  ...
  console.log(
    `Flags:         0x${FLAGS.toString(
      16
    )} (post-fee, balanceOf, bridge-amount-pos=4)`
  );
  ...
  logTxnSummary(
    `Polygon AAVE → Base USDC (CCTP) — swapAndBridge postFee/balanceOf`,
    CHAIN_IDS.POLYGON,
    receipt
  );
