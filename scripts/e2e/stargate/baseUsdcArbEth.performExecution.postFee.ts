 ...
 *
 * Usage:
 *   PRIVATE_KEY=0x... ts-node scripts/e2e/stargate/baseUsdcArbEth.performExecution.postFee.ts
 */
...
async function main() {
  ...
  const mono: MonolithicExecutionCall = {
    exec: {
      input: { user: signerAddress, inputToken: TOKENS.USDC_BASE, inputAmount },
      preFee: NO_FEE,
      swap: {
        target: ooRouter,
      ...
      bridge: { target: STARGATE_NATIVE_BASE, approvalSpender: ZERO_ADDRESS, value: nativeFeeWithBuffer },
  ...
  logTxnSummary(
    'Base USDC → ETH (OO) → Arbitrum ETH (Stargate native) — performExecution postFee',
    CHAIN_IDS.BASE,
    receipt,
  );
