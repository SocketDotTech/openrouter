/**
 * Route:  Mantle USDC → Base USDC (Stargate USDC Pool, no swap)
 ...
 *
 * Usage:
 *   PRIVATE_KEY=0x... ts-node scripts/e2e/stargate/mantleUsdcBase.bridge.preFee.ts
 */
...
async function main(): Promise<void> {
  ...
  const inputToken = TOKENS.USDC_MANTLE;
  const { balance: walletBalance } = await getWalletErc20Balance(inputToken, signerAddress, provider);
  if (walletBalance === 0n) {
    throw new Error(`Signer ${signerAddress} has zero USDC on Mantle`);
  }
  ...
  logTxnSummary('Mantle USDC → Base USDC (Stargate USDC pool) — bridge preFee', CHAIN_IDS.MANTLE, receipt);
