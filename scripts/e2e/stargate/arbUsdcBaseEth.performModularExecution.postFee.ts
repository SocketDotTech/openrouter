 ...
 *
 * Usage:
 *   PRIVATE_KEY=0x... ts-node scripts/e2e/stargate/arbUsdcBaseEth.performModularExecution.postFee.ts
 */
...
async function main() {
  ...
  exec.call(TOKENS.USDC_ARB, encodeApprove(ooRouter, inputAmount));
  ...
  const receipt = await execViaAH(signer, ROUTER_ARB, TOKENS.USDC_ARB, inputAmount, ROUTER_ARB, callData);

  logTxnSummary(
    'Arbitrum USDC → ETH (OO) → Base ETH (Stargate native) — performModularExecution postFee',
    CHAIN_IDS.ARBITRUM,
    receipt,
  );
