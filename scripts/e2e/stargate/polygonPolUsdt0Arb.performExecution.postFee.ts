 ...
 * swap.value = POL forwarded to OO router; bridge.value = nativeFeeWithBuffer (LZ fee).
 * Bridge amount position flag splices actual post-fee USDT0 balance at byte 196.
 *
 * For native-input cases this script must be run with sufficient POL balance.
 *
 * Usage:
 *   PRIVATE_KEY=0x... ts-node scripts/e2e/stargate/polygonPolUsdt0Arb.performExecution.postFee.ts
 */
...
async function main() {
  ...
  // Start with full usable balance; capped below if lz fee eats too much
  ...
  // Re-quote loop: cap inputAmountWei if balance can't cover lz fee + gas reserve
  ...
  logTxnSummary(
    'Polygon POL → USDT0 (OO) → Arbitrum USDT0 (OFT) — performExecution postFee',
    CHAIN_IDS.POLYGON,
    receipt,
  );
