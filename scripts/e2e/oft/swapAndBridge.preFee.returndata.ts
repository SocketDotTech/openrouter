 ...
 *
 * Usage:
 *   PRIVATE_KEY=0x... ts-node scripts/e2e/oft/swapAndBridge.preFee.returndata.ts
 */
...
// pre-fee (0x00) | bridge amount at byte offset 196 (sendParam.amountLD)
const FLAGS = bridgeAmountPositionFlag(196);
...
async function main() {
  ...
  console.log(
    `Flags:         0x${FLAGS.toString(
      16
    )} (pre-fee, returndata, bridge-amount-pos=196)`
  );
  ...
  const bridgeAmount = estimatedOut; // estimated bridge amount (no post-fee subtraction here)
  ...
  logTxnSummary(
    `Polygon AAVE → Arbitrum USDT0 (OFT) — swapAndBridge preFee/returndata`,
    CHAIN_IDS.POLYGON,
    receipt
  );
