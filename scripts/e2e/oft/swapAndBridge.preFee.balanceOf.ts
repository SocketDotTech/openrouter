 ...
 * Flags:  pre-fee (fee taken from AAVE input before swap), output measured as USDT0 balanceOf delta
 ...
 *
 * Usage:
 *   PRIVATE_KEY=0x... ts-node scripts/e2e/oft/swapAndBridge.preFee.balanceOf.ts
 */
...
async function main() {
  ...
  console.log(
    `Flags:         0x${FLAGS.toString(
      16
    )} (pre-fee, balanceOf, bridge-amount-pos=196)`
  );
  ...
  const bridgeAmount = estimatedOut;
  ...
  logTxnSummary(
    `Polygon AAVE → Arbitrum USDT0 (OFT) — swapAndBridge preFee/balanceOf`,
    CHAIN_IDS.POLYGON,
    receipt
  );
